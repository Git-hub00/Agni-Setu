...
| locust (backend dev group; see section 4 tooling) | current 2.x | **2.46.5** (B17) | MIT | see DEV-08 | dev; performance measurement only |
...
| uv distribution (api image build stage) | `ghcr.io/astral-sh/uv` matching the host uv | **0.11.28** | index `sha256:0f36cb9361a3346885ca3677e3767016687b5a170c1a6b88465ec14aefec90aa` (amd64 manifest `sha256:5c3ab83183a73c5d319a77009eb425b60d5bb937f339fb7876788ebf567baf48`) | amd64 (+ attestation manifest) | B18 (`docker buildx imagetools inspect`, 2026-09-12T01:21Z); `api.Dockerfile` `COPY --from` pinned by this digest |
| Trivy (release scanner, not shipped) | `aquasec/trivy` | **0.74.0** (observed from the `latest` tag on 2026-09-12; the release workflow pins `TRIVY_VERSION` to it) | container `sha256:62b1e65e8869bc4b4c6aa4fa2b21595256c7c2f6018a9d9ad61caf87187c1969` (the pulled `latest` image) | amd64 | B18 image SBOM + vulnerability scans (status21 s.3t); run from a container, never installed in an application image |
...
| DEV-08 | 2026-09-12 (B17) | add backend dev dependency **locust 2.46.5** (`>=2.32,<3`) | Test plan s.2/s.7 name Locust for the performance layer; `tests/load/locustfile.py` models the 70/20/10 workload; `scripts/ops/measure_load.py` runs it headless against the Compose stack | dev group only; `uv sync --frozen` reproduces; gevent/greenlet wheels resolved for Python 3.12 on Windows; measurement executed (status21 s.3s) | dev only; pip-audit re-run at B17 (result in s.3s); never installed in the api image (`--no-dev`) |
| DEV-09 | 2026-09-12 (B18) | add backend dev dependency **pyyaml 6.0.3** (`>=6,<7`; already present in the lock as a transitive dependency of the dev group) | `tests/unit/test_release_packaging.py` reads `compose.prod.yml` and `release.yml`; `scripts/ops/release_manifest.py` parses `pnpm-lock.yaml` for the web SBOM | `uv add --group dev` re-locked without any other change (126 packages resolved, nothing new installed); mypy override `yaml` (no stubs shipped) | dev only; never in the api image (`--no-dev`) |
