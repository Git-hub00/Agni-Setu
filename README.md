...
Demonstration acceptance: `uv run --directory backend python ../scripts/dev/acceptance_run.py` runs
all thirteen role-journey scripts against the stack in order and prints a PASS/FAIL table;
`docs/ACCEPTANCE_MATRIX.md` (generated by `scripts/ci/acceptance_matrix.py`) shows which of the
specified acceptance cases, properties, journeys and scenarios are automated, exercised by the
scripts, blocked or not run; `docs/23_PROTOTYPE_COVERAGE.md` s.7 records every prototype control's
implementation status and `docs/RELEASE_NOTES_DEMO.md` the demonstration's limitations.
